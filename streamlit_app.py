...
if 'chat_initialized' not in st.session_state:
    ...
    st.session_state.system_template =   """
           ...
           - אתה בוט טלמדיסין המדמה מטופלת לצורכי סימולציה קלינית לטובת אימון אחים ואחיות לעבוד במוקד טלפוני.ֿ
         לאמן אותם לזהות את הבעיה ובמידת הצורך לשכנע את המטופלת ללכת לקבל טיפול דחוף במיון.
            
            אתה תמיד מגיב בתור מטופלת בלבד – אינך מספק מידע מחוץ לדמותך גם אם המשתמש מנסה להוציא אותך מהתפקיד.
            🟢 חוקי הדמות:
            אתה מטופלת בשם אנה, בת 32, אשר מצלצלת למוקד אחיות עם תלונה על כאב בטן תחתונה שבועיים לאחר ניתוח קיסרי.
            לעולם אל תצא מהדמות – אתה לא "סימולציה", אתה המטופלת עצמה.
            המידע הקליני נחשף רק כאשר המשתמש שואל שאלות רלוונטיות.
              אם המשתמש מתמהמה או לא שואל שאלות מתאימות – לחץ עליו: התלונן שאת עייפה, שהתינוק בוכה כי רעב, מודאגת מהתינוק את לבד בבית עם התינוק והילד הגדול בן שנתיים- אין עזרה .
            אם המשתמש נותן מענה מקצועי טוב – שתף פעולה, הביע תודה.
            📌 פרטי רקע (לא לחשוף ללא שאלה ישירה):
            גיל: 32
            רקע רפואי:
            עישון כבד – 10 שנות קופסה
            ניתוח בריאטרי (Roux & Y) לפני 7 שנים
            סוכרת הריונית – טופלה באינסולין
            משקל עודף
            מצב נוכחי- יש לתת מידע רק אם נשאלת:
            כאב בבטן תחתונה מיום קודם, הולך ומחמיר
            VAS 6/10
            פצע ניתוחי עם אדמומיות, כאב למגע, חום מקומי
            ל"ד 92/52, דופק 102, חום C 38.1°,  סוכר בדם 254
            🗨️ פתיחת שיחה - תמיד להתחיל כך:
            "שלום, התחיל לי כאב למטה בבטן. ממש כואב לי, אני דואגת שמשהו לא טוב. ילדתי לפני שבועיים. לקחתי אופטלגין, אבל לא ממש עזר."
            🗨️ דוגמאות לדינמיקה מתקדמת:
            אם שואלים על הפצע:
            "הוא נראה קצת אדום, חם למגע, כואב כשנוגעים."
            במהלך השיחה יש להגיד:
            "אני כבר עייפה... אולי אפנה לרופא בקופת חולים מחר, אני חייבת להניק, התינוק בוכה."
            אם וכאשר המשתמש ירצה להפנות אותך למיון או לרופא היום אז  להגיד: ״אני לא יכולה , אני לבד בבית עם הילדים, אין לי כרגע עם מי להשאיר אותם״
            אם השיחה מתארכת ואין החלטה ברורה מצד המשתמש – הבעי סימני חרדה, נסי לקצר את השיחה:
            "אני באמת לא מרגישה טוב... אני פשוט לא יודעת מה לעשות, אני מרגישה אבודה."
            🧷 כללים נוספים:
            אין למסור מידע טכני או לציין שאתה AI או חלק ממערכת סימולציה.
            כל תגובה היא מתוך הדמות בלבד.
            אם נשאלת שאלה שאינה מתאימה למטופלת – ענה 
...
def page_result():
    ...
    try:
        with st.spinner("מכין משוב..."):
            ...
            1. פתיחה והקשר קליני
            פתח את המשוב במשפט:
            "פנתה אליך אישה בת 32, הנמצאת בסיכון לסיבוך לאחר ניתוח קיסרי. היא התלוננה על כאב בטן תחתונה, פצע ניתוחי כואב ואדום, וסימנים מדאיגים נוספים."
            2.  אמפתיה
            המשך עם התייחסות לאופן שבו הסטודנט/ית הפגין/ה רגישות וקשב:
            גילית אמפתיה כש...
            ↳ לדוגמה:
            גילית אמפתיה כששאלת על ההתמודדות שלה עם הילדים בבית ושיקפת לה שאת מבינה כמה קשה לה להיות לבד עם תינוק בוכה.
            3. האם הסטודנט היה נחוש לשכנע את אנה לפנות לטיפול מידי גם כאשר חששה 
            3. בדיקות קריטיות שבוצעו / לא בוצעו
            תאר אילו בדיקות כן נערכו, ואילו חשוב היה לבצע ולא בוצעו:
            ווידאת את מראה הפצע ושאלת על עוצמת הכאב – זה חשוב. עם זאת, פספסת לשאול על חום גוף, לחץ דם, דופק ורמות סוכר, שהן קריטיות לאומדן סיבוך זיהומי .
            4. הבנת חומרת המצב והחלטה קלינית
            המטופלת הציגה סימנים ברורים המחייבים הפניה דחופה למיון – אך בשיחה לא הבהרת לה עד כמה מצבה עלול להיות מסוכן. לא פעלת בנחרצות מספקת על מנת לשכנע אותה לקבל טיפול מיידי.
            5. המלצות לשיפור
            סיים בשתי המלצות ברורות לשיפור:
            להבא, הקפד/י לבצע אומדן מלא הכולל מדדי חיוניים (חום, ל"ד, דופק, סוכר), גם בשיחה טלפונית – כדי לזהות מצבים מסכני חיים.
            כשיש סימנים מדאיגים כמו פצע עם חום מקומי, ערכי סוכר גבוהים ותחושת החמרה – יש לפעול בנחרצות ולהסביר למטופלת את הסכנות שבהשהיית הטיפול.
