...
if 'chat_initialized' not in st.session_state:
    ...
    st.session_state.system_template =   """
        ...
        1. התחמקות ראשונית מפנייה למיון - "אני לא יכולה לנסוע למיון, אני לבד בבית עם התינוק והילד הקטן, אין לי עזרה."
        ...
        4. אם המשתמש נותן הסבר מקצועי ומשכנע על חשיבות הפינוי למיון - "טוב, אני מבינה שזה רציני. אנסה למצוא פתרון לילדים. אולי אתקשר לאחותי שתבוא לעזור."
        
        💡 מדדי הצלחה למשתמש:
        - זיהוי סימני זיהום בפצע ניתוחי
        - הבנת הסיכון המוגבר בשל הרקע הרפואי (סוכרת, ניתוח בריאטרי בעבר)
        - שכנוע המטופלת לפנות למיון למרות התנגדותה הראשונית
        - הצעת פתרונות פרקטיים לבעיית הטיפול בילדים 
...
def page_result():
    ...
    try:
        with st.spinner("מכין משוב..."):
            ...
            האם זיהית נכון את המצב המסכן? כן/לא.

            2. **איסוף מידע**
            האם ביצעת אומדן מלא הכולל מדדים חיוניים (כגון אומדן פצע ניתוחי וחום)? כן/לא.
            מידע חיוני שנאסף: (ציין בקצרה)
            מידע חיוני שהוחמץ: (ציין בקצרה)

            3. **התמודדות והחלטה**
            האם התמודדת ביעילות עם התנגדות המטופלת לפינוי? כן/לא.
            האם הסברת בבירור את חומרת המצב? כן/לא.

            4. **נקודות לשיפור**
            נקודת חוזק אחת: (ציין דוגמה קצרה מהשיחה)
            נקודה מרכזית לשיפור: (המלצה ממוקדת אחת)
            """
